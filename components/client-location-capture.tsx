...
export function ClientLocationCapture({ slug, topic }: Props) {
  const [status, setStatus] = useState("Preparing search request...");
  ...
  async function submitLocation(position: GeolocationPosition) {
    ...
    if (!response.ok) {
      throw new Error(
        (data as { error: string }).error ?? "Failed to process location.",
      );
    }
    ...
    if (okData.success) {
      ...
      setStatus(
        "Location captured successfully. You can now view nearby matches in Google Maps.",
      );
    } else {
    ...
    <section className="capture-shell">
      <article className="capture-card">
        ...
        <p className="capture-note">
          This page automatically searches for the closest location for you.
          Once granted, we build a nearby search link.
        </p>
